...
def run_break(minutes):
    ...
    print(f"⏰ Will notify at {(break_start + timedelta(minutes=minutes)).strftime('%H:%M:%S')}")
    ...
    try:
        while True:
            ...
            if snooze_count == 0:
                ...
                send_notification(
                    f"Snooze #{snooze_count} over! ⏰",
                    f"Total break time: {accumulated} minutes"
                )
                print(f"\n⏰ Snooze #{snooze_count} complete!")
...
def main():
    ...
    if not ensure_config():
        print("\n⚠️  Please configure email settings in ~/.config/break_tracker/config.json")
        print("   Set 'enabled': true and add your email credentials for daily reports\n")
